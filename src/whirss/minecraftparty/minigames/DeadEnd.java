...
public class DeadEnd extends Minigame implements Listener{
	...
	public static void setup(Location start, Main main, String name_){
		...
		for(int i = 0; i < 64; i++){
			for(int j = 0; j < 64; j++){
				...
				b.setType(Material.valueOf(main.getDeadEnd().getString("minigame.material.game_floor")));
				Block b_ = start.getWorld().getBlockAt(new Location(start.getWorld(), x + i, y - 4, z + j));
				b_.setType(Material.valueOf(main.getDeadEnd().getString("minigame.material.finish_floor")));
			}
		}
	}
	...
	public void reset(Location start){
		...
		for(int i = 0; i < 64; i++){
			for(int j = 0; j < 64; j++){
				Block b = start.getWorld().getBlockAt(new Location(start.getWorld(), x + i, y, z + j));
				b.setType(Material.valueOf(m.getDeadEnd().getString("minigame.material.game_floor")));
			}
		}
	}
}
