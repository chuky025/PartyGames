	...
	public static void setup(Location start, Main main, String name_){
		...
		for(int i = 0; i < 64; i++){
			for(int j = 0; j < 64; j++){
				...
				b_.setType(Material.valueOf(main.getRedAlert().getString("minigame.materials.finish_floor")));
				b.setType(Material.valueOf(main.getRedAlert().getString("minigame.materials.game_floor")));
			}
		}
	}
		...
		try{
			...
			for(int i = 0; i < 64; i++){
				for(int j = 0; j < 64; j++){
					...
					b.setType(Material.valueOf(m.getRedAlert().getString("minigame.materials.game_floor")));
